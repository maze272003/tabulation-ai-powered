      ...
      {targetPlan && targetPlan._id !== currentPlanId ? (
        <div className="p-4 bg-primary/10 border border-primary/25 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-sm">
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <span className="font-semibold text-foreground">Selected Plan: {targetPlan.name}</span>
              <p className="text-xs text-muted-foreground mt-0.5">
                You selected the {targetPlan.name} plan ({formatPeso(targetPlan.priceCents ?? 0)}/month). Complete checkout below to activate your subscription across all your organizations.
              </p>
            </div>
          </div>
          <Button
      ...
      {status === "past_due" ? (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-warning/40 bg-warning-muted px-4 py-3 text-sm text-warning">
          <span>
            ...
            period to keep your paid features across all your organizations.
          </span>
        </div>
      ) : null}
            ...
            <div className="flex flex-col h-full justify-between p-6">
              <div>
                ...
                <p className="text-xs font-medium text-foreground mb-3 pb-3 border-b border-border/60">
                  Up to {plan.limits.maxEvents} pooled event{plan.limits.maxEvents === 1 ? "" : "s"} ·{" "}
                  {plan.limits.maxJudges} pooled judges · {plan.limits.maxContestants} pooled contestants
                </p>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        icon={CreditCard}
        title="Billing"
        description="Your subscription covers every organization you create. Upgrade once, tabulate everywhere."
      />
