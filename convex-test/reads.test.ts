...
describe("reference reads & platform", () => {
  ...
  it("platform endpoints require platform owner", async () => {
    ...
    await expect(
      t.withIdentity(aliceIdentity).query(api.platform.orgs.list, {
  ...
  it("platform owner can list all orgs after bootstrap", async () => {
    ...
    // setPlatformRole requires an existing platform owner, so we patch the
    ...
    const result = await t.withIdentity(aliceIdentity).query(api.platform.orgs.list, {
      paginationOpts: { numItems: 10, cursor: null },
    });
