...
describe("seed", () => {
  ...
  it("refuses E2E seeding when the deployment token is not configured", async () => {
    const t = setupTest();
    await expect(t.mutation(api.seed.seedE2EData, { token: "anything" })).rejects.toThrow(
      /E2E seeding is not enabled/,
    );
  });
});
