 ...
 * post-login destinations. Anything else (absolute URLs, protocol-relative
 * URLs, unrelated paths) falls back to the default workspace route so the
...
function resolveNextParam(rawNext: string | null): string {
  ...
  return ALLOWED_NEXT_PREFIXES.some((prefix) => rawNext.startsWith(prefix)) ? rawNext : DEFAULT_NEXT;
}

export function SignInForm({ eventSessionToken }: { eventSessionToken: string | null }) {
  ...
  // A middleware bounce for a protected organization page (`next` set) always
  // requires an owner account — an event session can never satisfy it, so it
  // must not hijack that flow. Voluntary visits (no `next`) go straight to
  // the event workspace when a valid judge/staff session exists.
  ...
  const isCheckingSessions =
    isSessionPending || (eventSessionToken !== null && eventSession === undefined);
  ...
  if (isCheckingSessions) {
    return (
      <main className="min-h-screen bg-gradient-to-br from-background via-muted/30 to-background">
        <LoadingScreen label="Checking your session…" className="min-h-screen" />
      </main>
    );
  }
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    // A failed OAuth handshake would otherwise leave the button
                    // stuck in its pending state with no feedback.
